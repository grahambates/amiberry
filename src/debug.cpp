...
extern int debug_barto_cmd(TrapContext* ctx, uae_u32 arg1, uae_u32 arg2, uae_u32 arg3, uae_u32 arg4, uae_u32 arg5) {
	...
	case barto_cmd_register_resource: {
		...
		resource.address = uae_bswap_32(resource.address);
		resource.size = uae_bswap_32(resource.size);
		resource.type = uae_bswap_16(resource.type);
		resource.flags = uae_bswap_16(resource.flags);
		switch(resource.type) {
		case debug_resource_type_bitmap:
			resource.bitmap.width = uae_bswap_16(resource.bitmap.width);
			resource.bitmap.height = uae_bswap_16(resource.bitmap.height);
			resource.bitmap.numPlanes = uae_bswap_16(resource.bitmap.numPlanes);
			break;
		case debug_resource_type_palette:
			resource.palette.numEntries = uae_bswap_16(resource.palette.numEntries);
			break;
		}
	...
	case barto_cmd_load: {
		...
		trap_get_bytes(ctx, &resource, arg2, sizeof(resource));
		unsigned int address = uae_bswap_32(resource.address);
		std::string fn = std::string("debug\\") + resource.name;
		if(auto f = fopen(fn.c_str(), "rb")) {
			fseek(f, 0, SEEK_END);
			auto size = ftell(f);
			fseek(f, 0, SEEK_SET);
			resource.size = uae_bswap_32(size);
	...
	case barto_cmd_save: {
		...
		trap_get_bytes(ctx, &resource, arg2, sizeof(resource));
		unsigned int address = uae_bswap_32(resource.address);
		unsigned int size = uae_bswap_32(resource.size);
		// TODO:
...
/*static*/ bool debug_line (TCHAR *input) // BARTO
{
	TCHAR cmd, *inptr;
	uaecptr addr;
	bool err;
...
void debug_exception(int nr)
{
	if (debug_illegal) {
		if (nr <= 63 && (debug_illegal_mask & ((uae_u64)1 << nr))) {
			...
			barto_gdbserver::set_exception(nr); // BARTO
			activate_debugger();
		}
	}
